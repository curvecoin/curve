...
const char * const BITCOIN_CONF_FILENAME = "curve.conf";
const char * const BITCOIN_PID_FILENAME = "curved.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "curve" is a composite category enabling all Curve-related debug output
            if(ptrCategory->count(string("curve"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CurveCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CurveCore
    // Mac: ~/Library/Application Support/CurveCore
    // Unix: ~/.curvecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CurveCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CurveCore";
#else
    // Unix
    return pathRet / ".curvecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty curve.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override curve.conf
